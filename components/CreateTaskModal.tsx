...
const RichTextEditor = ({ label, value, onChange, placeholder, minHeight = "120px", required = false }: any) => {
  ...
  return (
    <div className="space-y-2 text-left">
      <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
        <span className={`size-1.5 rounded-full ${required ? 'bg-rose-500' : 'bg-primary/40'}`}></span>
        {label} {required && <span className="text-rose-500 text-[10px] lowercase font-bold">(obrigatório)</span>}
      </label>
      <div className={`border rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-primary/20 transition-all bg-white dark:bg-slate-900 shadow-sm ${required && !value.trim() ? 'border-rose-200 dark:border-rose-900/50' : 'border-slate-200 dark:border-slate-700'}`}>
        <div className="flex items-center gap-0.5 p-1.5 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-100 dark:border-slate-700">
          <button type="button" onClick={() => applyFormat('**', '**')} className="p-1.5 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors" title="Negrito">
            <span className="material-symbols-outlined text-[18px]">format_bold</span>
          </button>
          <button type="button" onClick={() => applyFormat('*', '*')} className="p-1.5 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors" title="Itálico">
            <span className="material-symbols-outlined text-[18px]">format_italic</span>
          </button>
          <div className="w-px h-4 bg-slate-200 dark:bg-slate-600 mx-1"></div>
          <button type="button" onClick={() => applyFormat('\n- ')} className="p-1.5 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors" title="Lista">
            <span className="material-symbols-outlined text-[18px]">format_list_bulleted</span>
          </button>
          <button type="button" onClick={() => applyFormat('\n- [ ] ')} className="p-1.5 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors" title="Checklist">
            <span className="material-symbols-outlined text-[18px]">checklist</span>
          </button>
          <button type="button" onClick={() => applyFormat('`', '`')} className="p-1.5 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors" title="Código">
        ...
        <textarea
          ...
          style={{ minHeight }}
          className="w-full p-4 bg-transparent border-none focus:ring-0 text-sm leading-relaxed resize-none placeholder:text-slate-300 dark:placeholder:text-slate-600 custom-scrollbar"
        />
      </div>
    </div>
  );
...
const CreateTaskModal: React.FC<CreateTaskModalProps> = ({ isOpen, onClose, onSubmit, sprints, epics, users, currentUser, okrs = [], keyResults = [] }) => {
  ...
  const initialDescriptionSections = {
    context: '',
    userStory: '',
    scenarios: '',
    todo: '',
  ...
  const [descriptionSections, setDescriptionSections] = useState(initialDescriptionSections);

  // Ordenação de Sprints
  ...
  const filteredEpics = useMemo(() => {
    ...
    const query = searchEpicQuery.toLowerCase();
    return epics.filter(e => e.title.toLowerCase().includes(query) || e.id.toLowerCase().includes(query));
  }, [epics, searchEpicQuery]);

  const filteredOkrs = useMemo(() => {
    if (!searchOkrQuery.trim()) return okrs;
    const query = searchOkrQuery.toLowerCase();
    return okrs.filter(o => o.title.toLowerCase().includes(query));
  }, [okrs, searchOkrQuery]);

  const filteredKrs = useMemo(() => {
    const relatedKrs = keyResults.filter(kr => kr.okrId === formData.okrId);
    if (!searchKrQuery.trim()) return relatedKrs;
    const query = searchKrQuery.toLowerCase();
    return relatedKrs.filter(kr => kr.title.toLowerCase().includes(query));
  ...
  const resetFields = () => {
    ...
    setDescriptionSections(initialDescriptionSections);
    setSelectedFiles([]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isEpic && !descriptionSections.context.trim()) {
      alert("O campo 'Contexto' é obrigatório para salvar a tarefa.");
      return;
    }

    if (isEpic && !formData.epicDescription.trim()) {
      alert("O campo 'Objetivo do Épico' é obrigatório.");
      return;
    }

    let fullDescription = '';
    if (isEpic) {
        fullDescription = formData.epicDescription;
    } else {
        fullDescription = [
          `### Contexto\n${descriptionSections.context}`,
          `### História do usuário\n${descriptionSections.userStory}`,
          `### Cenários\n${descriptionSections.scenarios}`,
          `### O que deve ser feito\n${descriptionSections.todo}`,
          `### Impacto com outros produtos\n${descriptionSections.impact}`,
          `### Critérios de aceite\n${descriptionSections.acceptance}`
        ].join('\n\n');
    }
  ...
  const updateSection = (key: keyof typeof descriptionSections, value: string) => {
    setDescriptionSections(prev => ({ ...prev, [key]: value }));
  };
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 w-full max-w-[950px] rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8 overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            <div className="space-y-2" ref={typeRef}>
              ...
              <div className="relative">
                ...
                {activeDropdown === 'type' && (
                  <div className="absolute top-full left-0 w-full mt-1 bg-white dark:bg-[#1a2330] border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[160] py-1 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="relative">
                <button type="button" onClick={() => setActiveDropdown(activeDropdown === 'resp' ? null : 'resp')} className="w-full h-11 px-4 flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm shadow-sm transition-all hover:border-primary/40">
                  <div className="flex items-center gap-3">
                    {selectedResp ? (
                      <>
                        <div className="size-6 rounded-full bg-cover bg-center border border-primary/20" style={{ backgroundImage: `url(${selectedResp.avatar})` }}></div>
                        <span className="text-slate-900 dark:text-white font-semibold">{selectedResp.name}</span>
                      </>
                    ) : (
                ...
                {activeDropdown === 'resp' && (
                  <div className="absolute top-full left-0 w-full mt-1 bg-white dark:bg-[#1a2330] border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[160] py-1 max-h-48 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2">
                    {users.map((user) => (
                      <button key={user.id} type="button" onClick={() => { setFormData({ ...formData, responsibleId: user.id }); setActiveDropdown(null); }} className={`w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700 text-left transition-colors ${formData.responsibleId === user.id ? 'bg-primary/5' : ''}`}>
                        <div className="size-8 rounded-full bg-cover bg-center border border-slate-100 dark:border-slate-800 shadow-sm" style={{ backgroundImage: `url(${user.avatar})` }}></div>
              ...
              <div className="relative">
                <button type="button" onClick={() => setActiveDropdown(activeDropdown === 'rep' ? null : 'rep')} className="w-full h-11 px-4 flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm shadow-sm transition-all hover:border-primary/40">
                  <div className="flex items-center gap-3">
                    {selectedRep ? (
                      <>
                        <div className="size-6 rounded-full bg-cover bg-center border border-primary/20" style={{ backgroundImage: `url(${selectedRep.avatar})` }}></div>
                        <span className="text-slate-900 dark:text-white font-semibold">{selectedRep.name}</span>
                      </>
                    ) : (
                      <span className="text-slate-400">Selecionar...</span>
                    )}
            ...
            <div className="space-y-2" ref={epicRef}>
              ...
              <div className="relative">
                <button 
                  ...
                  disabled={isEpic}
                  onClick={() => {
                    setActiveDropdown(activeDropdown === 'epic' ? null : 'epic');
                    setSearchEpicQuery('');
                  }} 
                  className={`w-full h-11 px-4 flex items-center justify-between rounded-lg border text-sm shadow-sm transition-all ${isEpic ? 'bg-slate-50 dark:bg-slate-800/30 border-slate-100 dark:border-slate-800 cursor-not-allowed' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-primary/40'}`}
                >
                   <div className="flex items-center gap-3">
                     <span className={`material-symbols-outlined text-[20px] ${selectedEpicTask ? 'text-purple-500' : 'text-slate-400'}`}>bolt</span>
                     <span className={`${selectedEpicTask ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-400'}`}>
                       {isEpic ? 'Hierarquia Máxima' : (selectedEpicTask ? selectedEpicTask.title : 'Nenhum')}
                     </span>
                   </div>
                ...
                {activeDropdown === 'epic' && !isEpic && (
                  <div className="absolute top-full left-0 w-full mt-1 bg-white dark:bg-[#1a2330] border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[160] py-1 max-h-64 overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-2">
                    <div className="p-2 border-b border-slate-100 dark:border-slate-800">
                      <div className="relative">
                        ...
                        <input 
                          type="text" 
                          placeholder="Pesquisar épico..." 
                          value={searchEpicQuery} 
                          onChange={e => setSearchEpicQuery(e.target.value)} 
                          className="w-full h-9 pl-8 pr-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-xs focus:ring-1 focus:ring-primary outline-none"
                        />
                      </div>
                    </div>
                        ...
                        filteredEpics.map(e => (
                          <button key={e.id} type="button" onClick={() => { setFormData({ ...formData, parentEpic: e.id }); setActiveDropdown(null); }} className={`w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700 text-left transition-colors ${formData.parentEpic === e.id ? 'bg-primary/5' : ''}`}>
                            ...
                            <div className="flex flex-col min-w-0">
                              <span className="text-[9px] font-bold text-slate-400 tracking-wider uppercase">{e.id}</span>
                              <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{e.title}</span>
                            </div>
                          </button>
                        ))
                      ) : (
                ...
                <div className="relative">
                  <button type="button" onClick={() => setActiveDropdown(activeDropdown === 'sprint' ? null : 'sprint')} className="w-full h-11 px-4 flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm shadow-sm transition-all hover:border-primary/40">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={selectedSprint ? 'text-slate-900 dark:text-slate-200 font-semibold' : 'text-slate-400'}>
                  ...
                  {activeDropdown === 'sprint' && (
                    <div className="absolute top-full left-0 w-full mt-1 bg-white dark:bg-[#1a2330] border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[160] py-1 max-h-64 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2">
                      ...
                      {availableSprintsOrdered.map(s => (
                        <button key={s.id} type="button" onClick={() => { setFormData({ ...formData, sprintId: s.id }); setActiveDropdown(null); }} className={`w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700 text-left transition-colors ${formData.sprintId === s.id ? 'bg-primary/5' : ''}`}>
                          ...
                          <div className="flex flex-col">
                            ...
                            {s.status === SprintStatus.ACTIVE && <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Sprint Ativa</span>}
                          </div>
                        </button>
                      ))}
                    </div>
          ...
          {/* NOVOS CAMPOS ESTRATÉGICOS (OKR E KR) */}
            ...
            <div className="space-y-2" ref={okrRef}>
              ...
              <div className="relative">
                <button 
                  type="button" 
                  onClick={() => {
                    setActiveDropdown(activeDropdown === 'okr' ? null : 'okr');
                    setSearchOkrQuery('');
                  }} 
                  className={`w-full h-11 px-4 flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm shadow-sm transition-all hover:border-primary/40`}
                >
                   <div className="flex items-center gap-3">
                     <span className={`material-symbols-outlined text-[20px] ${selectedOkr ? 'text-primary' : 'text-slate-400'}`}>track_changes</span>
                     <span className={`${selectedOkr ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-400'} truncate`}>
                       {selectedOkr ? selectedOkr.title : 'Vincular a um OKR...'}
                     </span>
                   </div>
                    ...
                    <div className="p-2 border-b border-slate-100 dark:border-slate-800">
                      <div className="relative">
                        ...
                        <input 
                          ...
                          value={searchOkrQuery} 
                          onChange={e => setSearchOkrQuery(e.target.value)} 
                          className="w-full h-9 pl-8 pr-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-xs focus:ring-1 focus:ring-primary outline-none"
                        />
                      </div>
                    </div>
                      ...
                      {filteredOkrs.length > 0 ? (
                        filteredOkrs.map(o => (
                          <button key={o.id} type="button" onClick={() => { setFormData({ ...formData, okrId: o.id, krId: '' }); setActiveDropdown(null); }} className={`w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700 text-left transition-colors ${formData.okrId === o.id ? 'bg-primary/5' : ''}`}>
                            ...
                            <div className="flex flex-col min-w-0">
                              <span className="text-[9px] font-bold text-slate-400 tracking-wider uppercase">{o.year}</span>
                              <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{o.title}</span>
                            </div>
                          </button>
                        ))
                      ) : (
              ...
              <div className="space-y-2 animate-in slide-in-from-right-2 fade-in" ref={krRef}>
                ...
                <div className="relative">
                  <button 
                    type="button" 
                    onClick={() => {
                      setActiveDropdown(activeDropdown === 'kr' ? null : 'kr');
                      setSearchKrQuery('');
                    }} 
                    className={`w-full h-11 px-4 flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm shadow-sm transition-all hover:border-primary/40`}
                  >
                     <div className="flex items-center gap-3">
                       ...
                       <span className={`${selectedKr ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-400'} truncate`}>
                         {selectedKr ? selectedKr.title : 'Selecionar KR...'}
                       </span>
                     </div>
                    ...
                    <div className="absolute top-full left-0 w-full mt-1 bg-white dark:bg-[#1a2330] border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[160] py-1 max-h-64 overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-2">
                      <div className="p-2 border-b border-slate-100 dark:border-slate-800">
                        <div className="relative">
                          ...
                          <input 
                            type="text" 
                            placeholder="Pesquisar KR..." 
                            value={searchKrQuery} 
                            onChange={e => setSearchKrQuery(e.target.value)} 
                            className="w-full h-9 pl-8 pr-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-xs focus:ring-1 focus:ring-primary outline-none"
                          />
                        </div>
                      </div>
                        ...
                        {filteredKrs.length > 0 ? (
                          filteredKrs.map(kr => (
                            <button key={kr.id} type="button" onClick={() => { setFormData({ ...formData, krId: kr.id }); setActiveDropdown(null); }} className={`w-full px-4 py-2.5 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700 text-left transition-colors ${formData.krId === kr.id ? 'bg-primary/5' : ''}`}>
                              ...
                              <div className="flex flex-col min-w-0">
                                <span className="text-[9px] font-bold text-slate-400 tracking-wider uppercase">{kr.priority}</span>
                                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{kr.title}</span>
                              </div>
                            </button>
                          ))
                        ) : (
          ...
          <div className="pt-6 border-t border-slate-100 dark:border-slate-800 space-y-10">
            {isEpic ? (
              <RichTextEditor required label="Objetivo do Épico" value={formData.epicDescription} onChange={(v: string) => setFormData({...formData, epicDescription: v})} placeholder="Visão estratégica deste épico..." minHeight="250px" />
            ) : (
              <div className="grid grid-cols-1 gap-12">
                <RichTextEditor required label="Contexto" value={descriptionSections.context} onChange={(v: string) => updateSection('context', v)} placeholder="Por que estamos fazendo?" />
                <RichTextEditor label="História do usuário" value={descriptionSections.userStory} onChange={(v: string) => updateSection('userStory', v)} placeholder="Como um usuário..." />
                <RichTextEditor label="Cenários" value={descriptionSections.scenarios} onChange={(v: string) => updateSection('scenarios', v)} placeholder="Dado que... Quando... Então..." />
                <RichTextEditor label="O que deve ser feito" value={descriptionSections.todo} onChange={(v: string) => updateSection('todo', v)} placeholder="Lista de tarefas..." />
                <RichTextEditor label="Impacto" value={descriptionSections.impact} onChange={(v: string) => updateSection('impact', v)} placeholder="Afeta outros sistemas?" />
                <RichTextEditor label="Critérios de aceite" value={descriptionSections.acceptance} onChange={(v: string) => updateSection('acceptance', v)} placeholder="- [ ] Critério..." />
              </div>
            )}
          </div>
          ...
          <div className="pt-6 border-t border-slate-100 dark:border-slate-800 space-y-4 text-left">
            ...
            <div className="flex flex-col gap-4">
              <button 
                ...
                className="w-full h-24 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center gap-2 hover:border-primary/50 hover:bg-primary/5 transition-all text-slate-400 hover:text-primary"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {selectedFiles.map((file, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 group">
                      ...
                      <button type="button" onClick={() => removeFile(i)} className="p-1.5 text-slate-300 hover:text-rose-500 transition-colors">
